...
select_files() {
    ...
    # Search in current, parent, and specific folders like Downloads, Videos, and Pictures for video files
    selected_files=$(find . ../.. ~/Downloads ~/Videos ~/Pictures -type f \( -iname \*.avi -o -iname \*.mp4 -o -iname \*.mkv -o -iname \*.flv -o -iname \*.webm \) | wofi --dmenu --multiple --prompt "Select video files")
    echo "$selected_files"
}
