...
def autostart():
#    home = os.path.expanduser('~/dotfiles/nixos/DE/qtile/autostart.sh')
#    subprocess.Popen([home])

# OnReload
def onreload():
 #   reload = os.path.expanduser('~/dotfiles/nixos/DE/qtile/reload.sh')
...
terminal = "kitty" if qtile.core.name == "wayland" else "kitty"
browser = "brave"
...
keys = [
    ...
    Key([mod], "u", lazy.spawn(ntCenter), desc="Exec notification center"),
...
screens = [
    Screen(
        wallpaper = f"~/dotfiles/wallpapers/{colors["theme"]}/active.jpg",
        wallpaper_mode = "fill",
        top=bar.Bar(
            widget_list,
            24,
